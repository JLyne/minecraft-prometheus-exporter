    ...
    protected static List<MetricFamilySamples> prefixFromCollector(Collector collector) {
        ...
        for (MetricFamilySamples mSample : collected) {
            ...
            for (Sample sample : mSample.samples) {
                samples.add(new Sample(AbstractMetric.prefix(sample.name), sample.labelNames, sample.labelValues, sample.value));
            }

            MetricFamilySamples prefixed = new MetricFamilySamples(AbstractMetric.prefix(mSample.name), mSample.type, mSample.help, samples);
            mfs.add(prefixed);
        }
        return mfs;
    }
